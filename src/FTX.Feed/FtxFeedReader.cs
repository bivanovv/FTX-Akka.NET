...
/// </summary>
public sealed class FtxFeedReader : UntypedActor, IWithUnboundedStash
{
    ...
    public FtxFeedReader(IFeedPublisherSettings settings, IActorRef publisherRef)
    {
        ...
        switch (message)
        {
            case WebSocketReceiveResult r:
                ...
                if (readResult.readOp == ReadResult.Close)
                {
                    ...
                    throw new FtxException("socket closed unexpectedly");
                }

                _publisherRef.Tell(new FtxFeedPublisher.SocketData(readResult.outputStr));
            ...
            case Status.Failure failure: // we had a socket failure
                ...
                throw failure.Cause == null
                    ? new FtxException("received socket failure on read")
                    : new FtxException("received socket failure on read", failure.Cause);
            case FtxFeedPublisher.ForciblyClose close:
                // we expect to be stopped and recreated by our parent under the circumstances
                throw new FtxException("forcibly closing connection.");
            default:
                Unhandled(message);
                break;
        }
    ...
    private CancellationTokenSource BeginRead(int index = 0)
    {
        ...
        var cts = new CancellationTokenSource(_heartbeatInterval * 2); // Make sure this is greater than the ping interval
    ...
    private void ShutdownIfPayloadTooLarge(int count)
    {
        _socket.CloseAsync(WebSocketCloseStatus.MessageTooBig, "message payload too large",
            CancellationToken.None);
        throw new FtxException(
            $"FTX sent {count} bytes, which exceeded our buffer size of {_buffer.Length}");
    }
