...
using FTX.Feed.Msgs;
using FTX.Messages;
using Error = FTX.Feed.Msgs.Error;
...
/// </summary>
public static class FtxFeedFlows
{
    ...
    private static IFeedMessage ParseFeedMessage(string input, JsonSerializerOptions jsonSerializerOptions, ILoggingAdapter? log = null)
    {
        ...
        if (rootElement.TryGetProperty("type", out var typeElement))
        {
            ...
            else if (document.RootElement.TryGetProperty("channel", out var channelElement))
            {
                ...
                {
                    case "fills":
                        msg = rootElement.Deserialize<SocketResponse<UserFill>>(jsonSerializerOptions)!;
                        break;
                    case "ticker":
                        msg = rootElement.Deserialize<SocketResponse<Ticker>>(jsonSerializerOptions)!;
                        break;
                    default:
