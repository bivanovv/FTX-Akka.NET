...
/// <summary>
///     Published used for GDAX live order book feed
/// </summary>
public sealed class GdaxFeedPublisher : ActorPublisher<string>, IWithUnboundedStash
{
    private readonly bool _authenticate;
    private readonly bool _heartbeat;

    private readonly IFeedPublisherSettings _settings;
    private readonly FtxClient _client;
    ...
    public GdaxFeedPublisher(GdaxRealtimeFeedClient client, TimeSpan connectTimeout, string channel, bool authenticate = false, bool heartbeat = true,
    ...
    public GdaxFeedPublisher(IFeedPublisherSettings settings)
    {
        ...
        _settings = settings;
        _client = settings.Client.HttpClient;
    ...
    {
        BecomeConnecting();
        _feedReader = SpawnGdaxFeedReader();
    }
    ...
    private bool Connecting(object message)
    {
        switch (message)
        {
            case GdaxFeedReader.Subscribed s:
    ...
    {
        if (terminated) // the GDAX Feed Publisher has already been terminated
        {
            _feedReader = SpawnGdaxFeedReader(); // create a new reader
            Stash.UnstashAll();
            Become(Connecting);
        }
    ...
    private IActorRef SpawnGdaxFeedReader()
    {
        var reader = Context.ActorOf(Props.Create(() =>
            new GdaxFeedReader(_settings, Self)), "reader");
        Context.Watch(reader);
        return reader;
    }

    /// <summary>
    ///     Acts as a completion signal for gracefully terminating the GDAX feed in the event that it needs to migrate.
    /// </summary>
    ...
    /// <summary>
    ///     INTERNAL API. Used for acquiring a reference to the <see cref="GdaxFeedReader" />
    /// </summary>
